#!/usr/bin/env python3
"""
JSON Template Page Creator
- Uses the EXACT JSON file from the user
- Preserves the complete beautiful layout and styling
- Only replaces placeholders with correct values
"""
...
def load_json_template() -> Optional[Dict]:
    """Load the exact JSON template file"""
    try:
        ...
        with open(JSON_TEMPLATE_FILE, 'r') as f:
            data = json.load(f)
            print(f"✅ JSON template loaded successfully")
            return data
...
def replace_placeholders_in_json(template_data: Dict, state_name: str, state_data: Dict) -> Dict:
    """Replace placeholders in the JSON structure, preserving the exact format"""
    print(f"Replacing placeholders in JSON template for {state_name}...")
    
    # First, convert the JSON to a string for easier processing
    ...
    replacements = {
        # Basic state info placeholders - USING LOWERCASE AS IN THE ORIGINAL JSON
    ...
    # Handle FAQ section - first find the FAQ placeholder pattern
    faq_pattern = r'<!-- FAQ_ITEMS -->'
    
    if faq_pattern in template_str:
        faqs = load_faqs()
        faq_content = create_faq_section(faqs, state_name, premium_rate)
        template_str = template_str.replace(faq_pattern, faq_content)
    ...
    if remaining_placeholders:
        ...
        for placeholder in unique_placeholders:
            ...
            template_str = template_str.replace(f"[{placeholder}]", state_name)
    else:
...
def create_state_page_from_json(state_name: str) -> Optional[int]:
    """Create a state page using the exact JSON template structure"""
    print(f"Creating EXACT page from JSON template for {state_name}...")
    ...
    try:
        ...
        # Load JSON template
        template_data = load_json_template()
        if not template_data:
            return None
        
        # Replace placeholders in the JSON structure
        processed_data = replace_placeholders_in_json(template_data, state_name, state_data)
        
        # Extract the content from the JSON structure
        # The structure might be {'data': {'page': content}} or similar
        content = None
        if 'data' in processed_data and isinstance(processed_data['data'], dict):
            first_key = next(iter(processed_data['data']))
            content = processed_data['data'][first_key]
...
def main():
    ...
    page_id = create_state_page_from_json(state_name)
    
    if page_id:
        print(f"\n✅ SUCCESS! Created the EXACT page for {state_name}.")
        print(f"View the page at: {BASE_URL}/?page_id={page_id}")
        print("This page will be EXACTLY as designed in your JSON template!")
    else:
