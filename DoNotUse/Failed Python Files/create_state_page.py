...
def replace_all_placeholders(content, state_name, state_data):
    """
    Comprehensive placeholder replacement function that handles all formats
    """
    print(f"Starting comprehensive placeholder replacement for {state_name}...")
    
    # First replace all variations of state name placeholders
    replacements = [
        ('[state_name]', state_name),
        ('%91state_name%93', state_name),
        ('Major Counties in [state_name]', f'Major Counties in {state_name}'),
        ('Find Bail Bonds in [state_name]', f'Find Bail Bonds in {state_name}'),
        ('Find Licensed [state_name]', f'Find Licensed {state_name}'),
        ('[state_name] Bail Bondsmen', f'{state_name} Bail Bondsmen'),
        ('[state_abbr]', state_data["abbreviation"]),
        ('%91state_abbr%93', state_data["abbreviation"])
    ]
    
    # Apply all state replacements
    ...
    if counties:
        # Replace numbered county placeholders
        for i, county in enumerate(counties[:3], 1):
            content = content.replace(f'[county_name_{i}]', county)
            content = content.replace(f'%91county_name_{i}%93', county)
            content = content.replace(f'[county_name]', county)
            content = content.replace(f'%91county_name%93', county)
            content = content.replace(f'[county_slug_{i}]', county.lower().replace(" ", "-"))
            content = content.replace(f'%91county_slug_{i}%93', county.lower().replace(" ", "-"))
    ...
    if faqs:
        ...
        for i, faq in enumerate(selected_faqs, 1):
            content = content.replace(f'[FAQ_QUESTION_{i}]', faq['question'])
            content = content.replace(f'[FAQ_ANSWER_{i}]', faq['answer'])
            content = content.replace(f'%91FAQ_QUESTION_{i}%93', faq['question'])
            content = content.replace(f'%91FAQ_ANSWER_{i}%93', faq['answer'])
            # Also replace non-numbered versions
            content = content.replace('[FAQ_QUESTION]', faq['question'])
            content = content.replace('[FAQ_ANSWER]', faq['answer'])
            content = content.replace('%91FAQ_QUESTION%93', faq['question'])
            content = content.replace('%91FAQ_ANSWER%93', faq['answer'])
    
    print("Completed placeholder replacement.")
    return content
...
def create_state_page(state_name):
    ...
    try:
        ...
        print("Starting placeholder replacement process...")
        
        # Use the comprehensive replacement function
        content = replace_all_placeholders(template_content, state_name, state_data)
        
        print("Creating page with replaced content...")
