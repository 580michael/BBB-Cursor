...
def generate_page_for_state(state_name, template_json):
    ...
    try:
        # Load state data
        state_data_path = os.path.join(STATE_DATA_DIR, f"{state_name.lower().replace(' ', '_')}.json")
        ...
        # Save to JSON file
        os.makedirs(GENERATED_PAGES_DIR, exist_ok=True)
        json_filename = os.path.join(GENERATED_PAGES_DIR, f"{state_name.lower().replace(' ', '_')}.json")
        ...
        html_filename = os.path.join(GENERATED_PAGES_DIR, f"{state_name.lower().replace(' ', '_')}.html")
