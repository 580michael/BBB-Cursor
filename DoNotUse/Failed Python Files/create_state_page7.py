#!/usr/bin/env python3
"""
State Page Creator for Bail Bonds Buddy (Version 7)
HYBRID APPROACH: Combines successful placeholder replacement from create_state_page2.py
with the correct formatting meta fields from newer versions.
"""
...
def create_faq_section(faqs: List[Dict[str, str]], state_name: str) -> str:
    ...
    for i, faq in enumerate(selected_faqs, 1):
        ...
        # Create accordion item - SIMPLE VERSION from create_state_page2.py
...
def simple_replace_placeholders(content: str, state_name: str, state_data: Dict) -> str:
    """
    SIMPLE version of placeholder replacement that worked in create_state_page2.py
    """
    print("Performing simple placeholder replacement...")
    ...
    for i, county in enumerate(state_data['counties'][:3], 1):
        ...
        # Also add a non-numbered version to catch generic [county_name]
    ...
    # Try different FAQ marker formats
    for marker in ['<!-- FAQ_ITEMS -->', '[et_pb_accordion_items]', '<!-- FAQ_SECTION -->']:
        if marker in content:
            print(f"Found FAQ marker: {marker}")
            content = content.replace(marker, faq_content)
    
    # Also handle direct FAQ placeholders
    for i in range(1, 6):
...
def create_state_page(state_name: str) -> Optional[int]:
    ...
    try:
        ...
        # Replace placeholders using the SIMPLE method from create_state_page2.py
        content = simple_replace_placeholders(template_content, state_name, state_data)
        
        # Prepare page data - with ALL meta fields for proper formatting
        page_data = {
