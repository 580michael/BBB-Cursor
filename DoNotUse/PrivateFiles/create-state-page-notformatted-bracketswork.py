...
def replace_all_placeholders(content, state_name, state_data):
    ...
    formats = {
        ...
        'url_encoded': lambda x: f'%91{x}%93',
        'curly': lambda x: f'{{{x}}}',
    }
    
    # 1. Replace State Name and Abbreviation
    state_replacements = {
        'state_name': state_name,
        'state_abbr': state_data['abbreviation'],
        'state': state_name,  # Handle shorter variant
    }
    
    for key, value in state_replacements.items():
        for fmt in formats.values():
            content = content.replace(fmt(key), value)
    
    # 2. Replace hardcoded state references
    content = content.replace('Oklahoma', state_name)
    content = content.replace('OK', state_data['abbreviation'])
    
    # 3. Replace common state name patterns
    patterns = [
        'Find Bail Bonds in',
        'Find Licensed',
        'Bail Bondsmen',
    ...
    for pattern in patterns:
        for fmt in formats.values():
            old = f"{pattern} {fmt('state_name')}"
            new = f"{pattern} {state_name}"
            content = content.replace(old, new)
    
    # 4. Replace map coordinates and address
    ...
    # 5. Use existing functions for specialized replacements
    ...
    print("Completed comprehensive placeholder replacement.")
    return content
