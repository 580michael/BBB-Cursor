...
def generate_page_for_state(state_name):
    ...
    # Save to file
    filename = f"/home/ubuntu/bailbonds/generated_pages/{state_name.lower().replace(' ', '_')}.html"
...
def generate_wordpress_import_file():
    ...
    generated_dir = "/home/ubuntu/bailbonds/generated_pages"
    state_files = [f for f in os.listdir(generated_dir) if f.endswith('.html')]
    ...
    with open(f"{generated_dir}/index.md", 'w') as f:
        f.write(index_content)
    
    print(f"WordPress import index created at {generated_dir}/index.md")

def main():
    """Main function to run the page generator"""
    # Check if state data exists
    if not os.path.exists("/home/ubuntu/bailbonds/state_data"):
